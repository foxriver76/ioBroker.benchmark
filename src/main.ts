...
class Benchmark extends utils.Adapter {
    ...
    private async runTests(selectedTests: string[]): Promise<void> {
        ...
        for (const activeTestName of selectedTests) {
            ...
            const actionsPerSecondStd = this.round(
                this.calcStd(times[activeTestName].map(time => this.config.iterations / time))
            );
            ...
            for (const [instance, result] of Object.entries(this.requestedMonitoring)) {
                ...
                summaryState.secondaries[instance] = {
                    ...
                    timeMean,
                    timeStd,
                    actionsPerSecondMean: this.round(
                        this.config.iterations / timeMean / Object.keys(this.requestedMonitoring).length
                    ), // actions are split on all instances
                    actionsPerSecondStd: this.round(
                        this.calcStd(
                            result.time.map(
                                time => this.config.iterations / time / Object.keys(this.requestedMonitoring).length
                            )
                        )
                    ),
                    epochs: this.config.epochs,
    ...
    private async onMessage(obj: ioBroker.Message): Promise<void> {
        ...
        if (!this.config.secondaryMode) {
            if (obj.command === 'test') {
                ...
                this.config = obj.message as ioBroker.AdapterConfig;
    ...
    private async checkRequirements(requirements: TestRequirements): Promise<void> {
        ...
        if (requirements.controllerVersion) {
            if (!this.controllerVersion) {
                throw new Error(
                    `Required js-controller version "${requirements.controllerVersion}", but current version is unknown`
                );
            }

            if (!semver.satisfies(this.controllerVersion, requirements.controllerVersion)) {
                throw new Error(
                    `Required js-controller version "${requirements.controllerVersion}", installed version "${this.controllerVersion}"`
                );
            }
        }
        ...
        if (requirements.freeMemory) {
            ...
            if (!state || typeof state.val !== 'number') {
                ...
                if (!state || typeof state.val !== 'number') {
                    throw new Error(
                        `Required free memory of ${requirements.freeMemory} MB, but was not able to determine free memory`
                    );
                } else {
                    memFree = state.val;
                }
            ...
            if (memFree < requirements.freeMemory) {
                throw new Error(
                    `Required free memory of ${requirements.freeMemory} MB, but only have ${memFree} MB left`
                );
            }
        }
    }
